...
FSpectral provides a computationally simpler route to the same goal:
...
Relative to Single, FpERM reduces average task RMSE by 45.0% when $r=2$ and 51.3% when $r=3$; paired calendar-block bootstrap intervals for these improvements exclude zero. Hard-share is best at $r=2$, whereas FpERM is best at $r=3$, showing that the value of exact versus approximate sharing can depend on the representation dimension. This empirical example supports the usefulness of representation sharing, but it does not by itself test contamination robustness; that claim is evaluated by the theory and controlled simulations above.
