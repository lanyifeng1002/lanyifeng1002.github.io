...
Under the low-dimensional representation introduced above, expanding the latent slope functions in the same basis as
$\boldsymbol\alpha^{(m)}(t)=\boldsymbol\Theta^{(m)}\boldsymbol\psi_K(t)$
gives $\boldsymbol B^{(m)}=\boldsymbol A^{(m)}\boldsymbol\Theta^{(m)}$.
Thus, after basis reduction, the low-dimensional representation structure is inherited directly by the finite-dimensional coefficient matrix.
